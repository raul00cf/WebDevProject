...
const ProfileUser = ({ option, setOption, setShowing }) => {
  ...
  const getInfo = (type) => {
    ...
    if (type === 'day' || type === 'month' || type === 'year') {
      if (user.birthday) {
        if (type === 'day') {
          return user.birthday.split('/')[0];
        }

        if (type === 'month') {
          return user.birthday.split('/')[1];
        }

        if (type === 'year') {
          return user.birthday.split('/')[2];
        }
      }
      else {
        return null;
  ...
  const saveInformation = () => {
    ...
    let newUsers = [...users];
    newUsers[session.user].name = name;
    newUsers[session.user].email = email;
    newUsers[session.user].gender = gender;
    newUsers[session.user].birthday = `${day}/${month}/${year}`;
    newUsers[session.user].phone = phone;
    newUsers[session.user].address = address;
    newUsers[session.user].password = password ? password : user.password;
      ...
      <Right maxWidth={ option==="history" ? "1200" : "1000" } >
        {option === "account" ? (
          ...
          <History>
            ...
            {users[session.user].history.map((item, id) => (
              <Line color={ id!==users[session.user].history.length-1 ? "grey" : "transparent" }>
                <Product>
                  <div id="container">
                    <div id="image">
