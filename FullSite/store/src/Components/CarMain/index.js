...
const CarMain = ({ car, setShowing }) => {
  ...
  const handleHorn = () => {
    let audio = new Audio(`http://${process.env.REACT_APP_IP}:5000/data/${car.audio}`);
  ...
  return (
    <Wrapper>
      <Content>
        <p id="name">
          { car.name } <StarRating rating={ car.rating } />
        </p>
        <img src={ `http://${process.env.REACT_APP_IP}:5000/data/${car.image}` } alt={ car.name } />
        <p id="stock" >
          { car.stock } in stock
        </p>
        <div class="select" >
